...
function guestStarsMap(stars) {
    const filteredStars = stars
        .filter(star => star.popularity >= 30)
        .map(star => {
            const characterWithoutParentheses = star.character.replace(/\([^)]*\)/g, '').trim();
            return star.name + " as " + characterWithoutParentheses;
        });

    return filteredStars.length > 0 ? filteredStars.join(", ") : ["None"];
}

async function grapher () {
    const title = localStorage.getItem('title');

    let response = await fetch(`https://api.themoviedb.org/3/search/tv?api_key=c4af9fa1b544f4c1176b81cd09f45ac0&query=${title}`);
    response = await response.json();
    const tmdbID = response.results[0].id
        
    let response2 = await fetch('https://api.themoviedb.org/3/tv/' + tmdbID + '?api_key=c4af9fa1b544f4c1176b81cd09f45ac0');
    ...
    for (let i = 1; i < seasons + 1; i++) {
        try {
            let fetchPromise = await fetch('https://api.themoviedb.org/3/tv/' + tmdbID + '/season/' + i + '?api_key=c4af9fa1b544f4c1176b81cd09f45ac0')
            response = await fetchPromise.json();
            console.log(response);
